...
import numpy as np
from dataset import VideoDataset_INF0
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim = None, groups = 8,is_temporal=False):
        super().__init__()
        ...
        if is_temporal:
            self.block3 = temporal_Block(dim_out,dim_out,groups = groups)
            self.block4 = temporal_Block(dim_out,dim_out,groups = groups)
...
class Unet3D(nn.Module):
    def __init__(
        ...
        #* 是否使用 temporal 方向的 3D convolution
        self.temporal_conv = temporal_conv

        block_klass = partial(ResnetBlock, groups = resnet_groups)
        block_klass_temporal = partial(block_klass, is_temporal = temporal_conv)
        block_klass_cond = partial(block_klass_temporal, time_emb_dim = cond_dim)
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        register_buffer('alpha_hat', alphas_cumprod)
    ...
    def ddim_sample(
        ...
        begin_img = normalize_img(begin_img)
        end_img = normalize_img(end_img)

        sample_img = torch.randn((batch_size, channels, num_frames, image_size, image_size), device=device)
        for i in tqdm(reversed(range(0, ddim_timesteps)), desc='sampling loop time step', total=ddim_timesteps):
            t = torch.full((batch_size,), ddim_timestep_seq[i], device=device, dtype=torch.long)
            prev_t = torch.full((batch_size,), ddim_timestep_prev_seq[i], device=device, dtype=torch.long)
            
            # 1. get current and previous alpha_cumprod
            alpha_cumprod_t = extract(self.alpha_hat, t, sample_img.shape)
            alpha_cumprod_t_prev = extract(self.alpha_hat, prev_t, sample_img.shape)
    
            # 2. predict noise using model
            # pred_noise = model(sample_img, begin_img,end_img,t,t_range)
            sample_img[:,:,0] = begin_img
            sample_img[:,:,-1] = end_img
        ...
        return unnormalize_img(sample_img)
    ...
    def p_losses(self, x_start, t, cond = None, noise = None, **kwargs):
        ...
        if self.loss_type == 'l1':
            ...
            loss = F.mse_loss(noise[:,:,1:-1], x_recon[:,:,1:-1])
        else:
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.model = torch.nn.DataParallel(diffusion_model).to("cuda")
        ...
        self.train_ds = VideoDataset_INF0(
                    dataset_dir,
                    seq_length,
                    height, 
                    width,
        ...
        # print(f"training dataset len: {len(self.train_ds)}")
        # print(f"val dataset len: {len(self.train_ds)}")
        assert len(self.train_ds) > 0, 'need to have at least 1 video to start training (although 1 is not great, try 100k)'

        self.train_dl = data.DataLoader(self.train_ds, batch_size = train_batch_size, shuffle=True, pin_memory=True,drop_last=True)
        self.val_dl = data.DataLoader(self.val_ds, batch_size = train_batch_size, shuffle=True, pin_memory=True,drop_last=True)
        self.opt = Adam(diffusion_model.parameters(), lr = train_lr)

        self.step = 0
        self.val_step = len(self.train_dl)
        # print(f"val every step: {len(self.train_dl)}")
        ...
        self.load_folder = Path(load_folder)

        os.makedirs(f"{results_folder}/logs", exist_ok=True)
        self.writer = SummaryWriter(log_dir=f"{results_folder}/logs")

        os.makedirs(f"{results_folder}/output", exist_ok=True)
    ...
    def load(self,**kwargs):
        ...
        # data = torch.load(str(self.load_folder / f'model-{milestone}.pt'))
        data = torch.load(f"{self.load_folder}/model.pt")
    ...
    def validation(
        ...
        mse_list = []
        pbar = tqdm(range(50), desc = "Val")
        idx = 0
        for i,video in enumerate(self.val_dl):
            loss = self.model(
                video,
        ...
        self.writer.add_scalar('loss/val',mean_mse,global_step=self.step)

        all_videos_list = self.ema_model.module.ddim_sample(
                                                begin_img = video[:,:,0],
                                                end_img = video[:,:,-1],
                                                batch_size=self.batch_size)
        video_path = f"{self.results_folder}/output/val"
        video_tensor_to_mp4(all_videos_list[0],video_path,self.step)
    ...
    def train(
        ...
        pbar = tqdm(range(self.save_and_sample_every), desc="Train")
        ...
        while self.step < self.train_num_steps:
            ...
            self.writer.add_scalar('loss/train',loss.item(),global_step=self.step)
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                all_videos_list = self.ema_model.module.ddim_sample(begin_img = video[:,:,0],
                                                                    end_img = video[:,:,-1],
                                                                    batch_size=self.batch_size)
                ...
                video_path = f"{self.results_folder}/output/train"
                video_tensor_to_mp4(all_videos_list[0],video_path,self.step)
